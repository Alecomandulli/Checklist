...
const App: React.FC = () => {
    ...
    const [activeTab, setActiveTab] = useState<'checklist' | 'analyzer' | 'history' | 'admin'>('checklist');
    ...
    useEffect(() => {
        ...
        // 1. Ensure default generic admin exists
        if (!users.some(u => u.cpf === 'admin')) {
             users.push({
        ...
        // 2. Ensure Specific Admin: Alessandro Comandulli
        const alessandroCpf = '60489596053';
        if (!users.some(u => u.cpf === alessandroCpf)) {
            users.push({
                name: 'Alessandro Comandulli',
        ...
        if (hasChanges) {
            localStorage.setItem('ambulanceUsers', JSON.stringify(users));
            console.log("Administrative users initialized/updated.");
        }
    }, []);

    // Load Login State
    useEffect(() => {
        const storedUser = localStorage.getItem('ambulanceLoggedInUser');
        if (storedUser) {
            const user = JSON.parse(storedUser);
            setCurrentUser(user);
            setCondutor(user.name);
            // If admin logs in, redirect to admin panel by default? Or let them choose.
            // Let's default to checklist but give option.
        }
    }, []);
    ...
    useEffect(() => {
        try {
            ...
            if (savedData) {
                ...
                if (headerData) {
                    if (!currentUser && headerData.condutor) setCondutor(headerData.condutor);
    ...
    // Update condutor when user logs in
    useEffect(() => {
        if (currentUser) {
            setCondutor(currentUser.name);
        }
    }, [currentUser]);
    ...
    const handleLogin = (user: User) => {
        setCurrentUser(user);
        localStorage.setItem('ambulanceLoggedInUser', JSON.stringify(user));
        if (user.role === 'admin') {
            setActiveTab('admin');
        } else {
    ...
    const handleLogout = () => {
        if (window.confirm('Deseja realmente sair?')) {
            setCurrentUser(null);
            localStorage.removeItem('ambulanceLoggedInUser');
            setCondutor('');
            setItemStates({});
            setActiveTab('checklist');
        }
    ...
    const saveToHistory = () => {
        if (!condutor || !vtr) {
            alert("Por favor, preencha o número da VTR para salvar no histórico.");
            return;
        }
                ...
                <div className="max-w-4xl mx-auto flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-gray-400 text-xs sm:text-sm">
                            {currentUser.name} {currentUser.role === 'admin' ? '(Administrador)' : ''}
                        </p>
                    </div>
                    <div className="flex items-center gap-4">
                        {currentUser.role === 'admin' && (
                            <button 
                                ...
                                className="text-sm bg-purple-800 hover:bg-purple-700 px-3 py-1 rounded-md transition hidden sm:block"
                            >
                                Painel Admin
                            </button>
                        )}
                        <button 
                            ...
                            className="p-2 text-gray-400 hover:text-red-400 hover:bg-gray-800 rounded-full transition"
                            title="Sair"
                        >
                ...
                {activeTab === 'checklist' && (
                    <div className="bg-gray-800 p-4 rounded-xl shadow-lg mb-6">
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                            <InfoInput id="condutor" label="Condutor" value={condutor} onChange={(e) => setCondutor(e.target.value)} readOnly={true} />
